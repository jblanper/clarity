...
- **Moment chip → inline edit card** — further revised: tapping a chip now replaces it in-place within the flex grid with a full-width edit card (`w-full INLINE_FORM_SHELL`). All other chips and the `+ New` chip remain visible. The `+ New` chip only hides when the add-new form is open (not when editing an existing chip). This is a cleaner spatial interaction — the form appears at the chip's position in the grid flow rather than below it.
