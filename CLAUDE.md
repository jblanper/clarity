...
- **ManageView** — all inline editors are mutually exclusive via `closeAllEditors()`. Archive buttons use `text-amber-700` (reversible, not destructive). The add-habit form and the inline edit form are separate JSX subtrees with different indentation — when editing label copy in one, always verify the other is updated too. Active habit rows use a full-width tap button that reveals an **action tray** (`actionTrayId` state); the tray animates via `height: 0 → "auto"` + opacity at 220ms ease-out with `paddingBottom: 0` in the exit to prevent snap. The tray is hidden while `editingHabit` is set. Moments use a chip grid with `editingMomentId`/`editingMomentLabel` for in-place label editing.
