<!-- Last reviewed: 2026-03-20 (update-claude-md, sprint-15-retro) -->
...
lib/          storage.ts · habitConfig.ts · transferData.ts · habits.ts · theme.ts · calendarUtils.ts · *.test.ts
...
- **HeatmapFilter** — exported type `{ type: "boolean-habit" | "numeric-habit" | "moment"; id: string }` from `CalendarHeatmap.tsx`. When set, non-matching cells drop to `opacity-30` on the outer button; no other filter signal. Filter is owned by `HistoryView` state and passed to both `CalendarHeatmap` and `FrequencyList`.
