 ...
 * Groups skills by common naming prefix (discovered dynamically).
...
for (const [origin, skillsDir] of [
  ...
  for (const skill of entries) {
    ...
    if (!fm) {
      ...
      if (!fm['description']) {
        issues.push({ severity: 'warning', message: 'Frontmatter missing `description` — Claude will read the first paragraph instead' });
      }
      ...
      if (!fm['allowed-tools']) {
        issues.push({ severity: 'info', message: 'No `allowed-tools` constraint — skill can use all tools (security gap)' });
      }
    }
